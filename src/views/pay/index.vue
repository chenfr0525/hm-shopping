...
export default {
  ...
  methods: {
    ...
    async submitOrder () {
      let res
      if (this.mode === 'cart') {
        res = await checkOrder(this.mode, { cartIds: this.cartIds, remamrk: this.remark })
      } else {
        res = await checkOrder(this.mode, { goodsId: this.goodsId, goodsNum: this.goodsNum, goodsSkuId: this.goodsSkuId, remamrk: this.remark })
      }
      if (res.status === 200) {
        console.log(res)
